...
void sqa_stats_add_sample(struct sqa_stats *statistics, struct timespec *delay)
{
	...
	if (delay->tv_sec > statistics->delay_eq_loss_threshold.tv_sec ||
		...
		// If delay
		if (statistics->max.tv_sec == 0 &&
		    statistics->max.tv_nsec == 0) {
		...
		double delay_in_seconds =
			(double)delay_usec / (double)USEC_IN_SEC;

		statistics->shifted_sum +=
			delay_in_seconds - statistics->offset;
		statistics->shifted_sum_of_squares += pow(
			delay_in_seconds - statistics->offset, SQUARE_EXPONENT);
...
{
	if (statistics->number_of_samples ==
	    statistics->number_of_lost_packets) {
...
double sqa_stats_get_mean(struct sqa_stats *statistics)
{
	...
	return statistics->offset +
	       (statistics->shifted_sum / (statistics->number_of_samples -
					   statistics->number_of_lost_packets));
}
...
double sqa_stats_get_variance(struct sqa_stats *statistics)
{
	...
	int number_of_latency_samples = statistics->number_of_samples -
					statistics->number_of_lost_packets;
			 ...
			 struct simple_NR_list *nr)
{
	...
	for (int i = 0; i < nr->nr_perf.num_percentiles; i++) {
		...
		double qoo_part =
			(1 - (((mes_lat)-nrp_lat) / (nrpou_lat - nrp_lat))) *
			MAX_PERCENTILE;
		if (qoo_part < 0) {
			qoo_part = 0;
		}
